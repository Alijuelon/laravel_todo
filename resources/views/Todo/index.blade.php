        ...
        <div class="card shadow-sm border-0 rounded-3">
                ...
                <div class="container-fluid">
                    ...
                    <form class="d-flex" role="search" method="GET" action="{{ route('todo.index') }}">
                        <div class="input-group">
                            <input class="form-control" type="search" placeholder="Search todos..." name="search"
            ...
            <div class="card-body">
                <div class="d-flex justify-content-between align-items-center mb-4">
                    ...
                    <a href="{{ route('todo.tambah') }}" class="btn btn-primary">
                            ...
                            <div class="card h-100 shadow-sm mt-2 mb-2">
                                    ...
                                    <div class="col-md-8">
                                        <div class="card-body">
                                            ...
                                            <span class="badge {{ $todo->completed ? 'bg-success' : 'bg-warning' }}">
                                                <i
                                                    class="fas {{ $todo->completed ? 'fa-check-circle' : 'fa-clock' }} me-1"></i>
                                                {{ $todo->completed ? 'Completed' : 'Pending' }}
                                            </span>
                                            <p class="text-muted mt-2">
                                                <i class="fas fa-calendar me-2"> Di Buat</i>
                                                {{ \Carbon\Carbon::parse($todo->created_at)->format('d M Y, H:i') }}
                                            </p>
                                            <p class="text-muted mt-2">
                                                <i class="fas fa-calendar me-2"> Selesai </i>
                                                {{ \Carbon\Carbon::parse($todo->update_at)->format('d M Y, H:i') }}
                                            </p>
                                        </div>
                                    </div>
                                    <div class="col-md-4">
                                        @if($todo->documentation)
                                            <img src="{{ asset('storage/' . $todo->documentation) }}"
                                                class="img-fluid rounded-end h-100 object-fit-cover img-thumbnail"
                                                alt="Documentation">
                                        @else
                                            <div
                                                class="h-100 d-flex align-items-center justify-content-center bg-light rounded-end">
                                ...
                                <div class="card-footer bg-transparent">
                                    <div class="btn-group w-100 mt-1 d-flex justify-content-center">
                                        <!-- Tombol Edit -->
                                        <button type="button" class="btn btn-outline-primary btn-sm px-0 py-1"
                                            data-bs-toggle="modal" data-bs-target="#editModal{{ $todo->id }}">
                                            <i class="fas fa-edit fa-xs"> Edit </i>
                                        </button>
                                        <!-- Tombol Hapus -->
                                        <button type="button" class="btn btn-outline-danger btn-sm px-0 py-1"
                                            onclick="confirmDelete('{{ $todo->id }}')">
                                            <i class="fas fa-trash fa-xs"> Hapus </i>
                                        </button>
                                    </div>
                                    <form id="deleteForm-{{ $todo->id }}" action="{{ route('todo.destroy', $todo->id) }}"
                                        ...
                                        <div class="modal-body">
                                            ...
                                            <div class="form-check">
                                                ...
                                                <label class="form-check-label">
                                                    <i class="fas fa-check-circle me-1"></i>Centang Untuk Menyelesaikan
                                                </label>
                                            </div>
                                        </div>
